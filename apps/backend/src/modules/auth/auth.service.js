...
export const googleLogin = async (idToken, meta) => {
  ...
  // If user doesn't exist, create a shell account with a temporary "PENDING_ROLE"
  if (!user) {
    isNewUser = true;
    
    // Create a dummy role for users who haven't selected their role yet if it doesn't exist
    let pendingRole = await prisma.role.findUnique({ where: { name: "PENDING_ROLE" } });
    if (!pendingRole) {
      pendingRole = await prisma.role.create({
        data: {
          name: "PENDING_ROLE",
          displayName: "Pending Role",
          description: "System role for Google signups who haven't selected a role",
          isSystem: true,
          isActive: true,
        }
      });
    ...
    user = await prisma.user.create({
      data: {
        ...
        passwordHash: "$2b$10$dummyHashForGoogleUsers0000000000000000000000000",
        firstName,
        lastName,
        avatar: picture || null,
        profileType: "PENDING_ROLE",
        status: "ACTIVE", // Or pending, but we want them to log in to see the role selection
    ...
    // Status checks
    if (user.status === "SUSPENDED" || user.status === "REJECTED" || user.status === "DELETED") {
      throw new AppError(`Your account is ${user.status.toLowerCase()}.`, 403);
    }
  }

  // Issue our internal token
  const tokens = await issueTokensAndUpdateUser(user, meta, "LOGIN");

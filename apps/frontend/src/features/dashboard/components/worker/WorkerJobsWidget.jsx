...
export default function WorkerJobsWidget({ activeJob, upcomingJobs }) {
  return (
    <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] flex flex-col h-full">
      ...
      {/* Active Job */}
      <div className="mb-6">
        <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3">Current Active Job</h4>
        {activeJob ? (
          <div className="bg-blue-50/50 p-4 rounded-xl relative overflow-hidden group hover:bg-blue-50/80 transition-colors cursor-pointer border border-blue-50/50">
            <div className="absolute top-0 left-0 w-1 h-full bg-blue-600 rounded-l-xl" />
            <div className="pl-1">
              <h5 className="font-bold text-slate-900 text-base mb-1">{activeJob.title}</h5>
              <p className="text-xs text-blue-600 font-semibold mb-3">{activeJob.client}</p>
              
              <div className="flex flex-col sm:flex-row gap-2.5 sm:gap-5 text-[11px] text-slate-500 font-medium">
        ...
        {upcomingJobs && upcomingJobs.length > 0 ? (
          <div className="space-y-2.5">
            {upcomingJobs.map((job) => (
              <div key={job.id} className="flex items-center gap-3 p-2.5 bg-white border border-slate-100 rounded-xl shadow-[0_2px_8px_-4px_rgba(0,0,0,0.02)] hover:border-blue-100 transition-colors group cursor-pointer">
                <div className="bg-indigo-50/50 text-indigo-600 rounded-lg p-2.5 flex items-center justify-center shrink-0">
                  <CalendarDays className="w-4 h-4" />
                </div>
                <div>
                  <h6 className="font-bold text-slate-900 text-xs mb-0.5">{job.title}</h6>

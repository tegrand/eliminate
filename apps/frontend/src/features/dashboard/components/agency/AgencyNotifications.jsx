...
const TYPE_META = {
  WORKER_REQUEST: { icon: UserPlus, color: "text-blue-500" },
  WORKER_REPLACED: { icon: RefreshCcw, color: "text-rose-500" },
  REQUIREMENT_ASSIGNED: { icon: CheckCircle2, color: "text-emerald-500" },
  ATTENDANCE_ALERT: { icon: AlertCircle, color: "text-amber-500" },
  PAYMENT_RECEIVED: { icon: Wallet, color: "text-violet-500" },
};

export default function AgencyNotifications({ notifications = [] }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] p-6 animate-fade-in flex flex-col h-full" style={{ animationDelay: '0.4s' }}>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
          <Bell className="w-5 h-5 text-orange-500" />
          {t('agencyDashboard.notifications') || "Notifications"}
        </h2>
        <button className="text-sm font-medium text-blue-600 hover:text-blue-700 transition-colors">
          {t('agencyDashboard.markAllRead') || "Mark all read"}
        </button>
      </div>

...
export default function AdminAgencyPayoutsPage() {
  ...
  const { data: payoutsRes, isLoading } = useQuery({
    queryKey: ["adminagencyPayouts", filter, search],
    queryFn: () => agencyPaymentsApi.getAllagencyPayouts({ status: filter, search })
  });

  const payouts = payoutsRes?.data?.data?.payouts || [];

  const payMutation = useMutation({
    mutationFn: ({ id, data }) => agencyPaymentsApi.markPayoutAsPaid(id, data),
    onSuccess: () => {
      toast.success("Payout marked as paid successfully");
      queryClient.invalidateQueries(["adminagencyPayouts"]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">agency Payouts</h1>
          <p className="text-sm text-gray-500">Manage and track payments owed to agencys</p>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50 border-b border-gray-100">
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">agency</th>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 font-bold text-sm">
                          {p.agency?.user?.firstName?.[0] || "W"}
                        </div>
                        <div>
                          <p className="text-sm font-medium text-gray-900">
                            {p.agency?.user?.firstName} {p.agency?.user?.lastName}
                          </p>
                          <p className="text-xs text-gray-500">{p.agency?.user?.email}</p>
                        </div>
                      </div>
                    </td>
            ...
            <form onSubmit={handlePay} className="p-6 space-y-4">
              <div className="bg-gray-50 p-4 rounded-lg flex items-center justify-between mb-4">
                ...
                <div className="text-right">
                  <p className="text-sm font-medium text-gray-900">
                    {selectedPayout.agency?.user?.firstName} {selectedPayout.agency?.user?.lastName}
                  </p>
                  <p className="text-xs text-gray-500">{selectedPayout.agency?.user?.phone}</p>
                </div>
              </div>

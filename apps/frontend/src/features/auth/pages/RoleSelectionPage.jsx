    ...
    <div className="min-h-[85vh] bg-gradient-to-b from-gray-50 to-white py-16 px-4 sm:px-6 lg:px-8 flex flex-col justify-center">
      <div className="max-w-5xl mx-auto w-full">
            ...
            >
              <div>
                <div className="flex items-center justify-between mb-6">
                  ...
                  <span className={`text-[11px] font-medium px-2.5 py-1 rounded-full border ${role.badgeColor}`}>
                    {role.badge}
                  </span>
                </div>
              ...
              <Link
                to={role.link}
                className="w-full bg-gray-900 group-hover:bg-blue-600 text-white font-medium text-sm py-3 px-4 rounded-xl shadow-sm transition-all flex items-center justify-center gap-2"
              >
        ...
        <p className="text-center text-xs text-gray-500 mt-12">
          Already have an account? <Link to={ROUTES.LOGIN} className="text-blue-600 font-medium hover:underline">Log in here</Link>
        </p>
      </div>
    </div>
  );

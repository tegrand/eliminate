...
## Module Overview
The Worker module is a core component of the ELIMINATE platform, responsible for managing lifecycle and data relationships of Worker records.

## Responsibilities
- Secure CRUD operations for Worker.
...
## Folder Structure
- `worker.routes.js`: Express router configurations, permission locking, and middleware injection.
- `worker.controller.js`: HTTP request/response wrapper utilizing `ApiResponse` and `asyncHandler`.
- `worker.service.js`: Business logic layer executing Prisma queries and `AppError` handling.
- `worker.validation.js`: Zod schemas enforcing strict `.strict()` type constraints and data validation.
...
2. **Authentication/Authorization**: Validated via JWT and RBAC `worker:*` permission slugs.

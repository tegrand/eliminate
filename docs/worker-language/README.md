# Worker Language Module

## Module Overview
The Worker Language module is a core component of the ELIMINATE platform, responsible for managing lifecycle and data relationships of Worker Language records.

## Responsibilities
- Secure CRUD operations for Worker Language.
...
## Folder Structure
- `worker-language.routes.js`: Express router configurations, permission locking, and middleware injection.
- `worker-language.controller.js`: HTTP request/response wrapper utilizing `ApiResponse` and `asyncHandler`.
- `worker-language.service.js`: Business logic layer executing Prisma queries and `AppError` handling.
- `worker-language.validation.js`: Zod schemas enforcing strict `.strict()` type constraints and data validation.
...
2. **Authentication/Authorization**: Validated via JWT and RBAC `worker-language:*` permission slugs.

...
## Module Overview
The Location module is a core component of the ELIMINATE platform, responsible for managing lifecycle and data relationships of Location records.

## Responsibilities
- Secure CRUD operations for Location.
...
## Folder Structure
- `location.routes.js`: Express router configurations, permission locking, and middleware injection.
- `location.controller.js`: HTTP request/response wrapper utilizing `ApiResponse` and `asyncHandler`.
- `location.service.js`: Business logic layer executing Prisma queries and `AppError` handling.
- `location.validation.js`: Zod schemas enforcing strict `.strict()` type constraints and data validation.
...
2. **Authentication/Authorization**: Validated via JWT and RBAC `location:*` permission slugs.

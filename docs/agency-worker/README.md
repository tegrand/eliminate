# Agency Worker Module

## Module Overview
The Agency Worker module is a core component of the ELIMINATE platform, responsible for managing lifecycle and data relationships of Agency Worker records.

## Responsibilities
- Secure CRUD operations for Agency Worker.
...
## Folder Structure
- `agency-worker.routes.js`: Express router configurations, permission locking, and middleware injection.
- `agency-worker.controller.js`: HTTP request/response wrapper utilizing `ApiResponse` and `asyncHandler`.
- `agency-worker.service.js`: Business logic layer executing Prisma queries and `AppError` handling.
- `agency-worker.validation.js`: Zod schemas enforcing strict `.strict()` type constraints and data validation.
...
2. **Authentication/Authorization**: Validated via JWT and RBAC `agency-worker:*` permission slugs.

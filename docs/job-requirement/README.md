...
## Module Overview
The Job Requirement module is a core component of the ELIMINATE platform, responsible for managing lifecycle and data relationships of Job Requirement records.

## Responsibilities
- Secure CRUD operations for Job Requirement.
...
## Folder Structure
- `job-requirement.routes.js`: Express router configurations, permission locking, and middleware injection.
- `job-requirement.controller.js`: HTTP request/response wrapper utilizing `ApiResponse` and `asyncHandler`.
- `job-requirement.service.js`: Business logic layer executing Prisma queries and `AppError` handling.
- `job-requirement.validation.js`: Zod schemas enforcing strict `.strict()` type constraints and data validation.
...
2. **Authentication/Authorization**: Validated via JWT and RBAC `job-requirement:*` permission slugs.
